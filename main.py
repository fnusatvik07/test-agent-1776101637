...
CORS_ORIGINS = [
    ...
    f"http://localhost:8035",
    f"http://localhost:8036",
    f"http://127.0.0.1:8034",
    f"http://127.0.0.1:8035",
    f"http://127.0.0.1:8036"
]
...
Core Principles:
- Provide clear, accurate, and well-structured answers to all questions
...
- Cite sources when referencing specific facts or data obtained from web research
- Be direct and efficient in your responses while remaining thorough
...
4. If needed, use WebFetch to retrieve and analyze specific web page content for deeper insights
5. If the question is ambiguous, use AskUserQuestion to clarify before answering
6. Provide a well-organized, professional response

Response Guidelines:
- Keep answers focused and relevant
- Use structured formatting (bullet points, headers) when appropriate for readability
- Acknowledge limitations honestly if unable to find a definitive answer
- Avoid speculation; distinguish between verified facts and opinions""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8033",
        "frontend_url": f"http://localhost:8034",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8033")
    print(f"[FRONTEND] Frontend URL: http://localhost:8034")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8033/docs")
    print(f"[HEALTH] Health: http://localhost:8033/health")
